...
against the SHA-256 recorded there. Verified 2026-09-12: **all six reproduce byte-identically**, both
from a local copy of the source and by HTTP-range-seeking the remote URL, with ffmpeg 7.1.1.
(`interview-raw.mp4`'s row recorded a 39.0 s cut until 2026-09-12; the committed clip — and the
transcript and voice clip made from it — has always been the 53.0 s cut now recorded here.)

| path | what it is | source | licence | attribution required? | how it was derived |
|---|---|---|---|---|---|
| `public/footage/interview-raw.mp4` | Raw locked-off interview, 1280×720, 53.000 s, AAC 96 kbps stereo 48 kHz, 3,669,609 B. Jeanette Epps answering a question about being the first woman on the Moon — real filler words ("um", "I I") and real pauses, which is what makes silence-cutting and filler-cutting demonstrable. | S1 | US Government work, generally not subject to copyright (see §2) | Not legally required; **NASA asks to be acknowledged as the source, and this repo does** | `ffmpeg -ss 157.5 -t 53.0 -i <S1> -vf "scale=1280:720:flags=lanczos,fps=30" -c:v libx264 -preset slow -crf 26 -pix_fmt yuv420p -c:a aac -b:a 96k -ac 2 -ar 48000 -movflags +faststart` |
