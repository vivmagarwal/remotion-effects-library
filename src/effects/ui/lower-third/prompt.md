...
- **Accent bar**: 22px wide, `#ff5c39`, `transformOrigin: 'bottom left'`, `scale: \`1 ${bar}\`` — it
  grows vertically out of nothing.
- **Plate**: `#0f1116`, `overflow: hidden`, `transformOrigin: 'left center'`,
...
- Inside: a `LIVE` chip (20px weight 800, `letter-spacing: 0.18em`, dark text on the accent, radius 5),
  the name at 52px weight 800, and a title line at 26px weight 500 in `#9aa2b4`.
- A 3px accent rule 14px below the plate, growing 0→620px on the `text` stage.
...
- With `transparent`, render as an alpha overlay (`--codec=vp8`, or
  `--codec=prores --prores-profile=4444`) and composite over your footage — which is how a real lower
  third gets used.
