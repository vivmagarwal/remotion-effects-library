...
export const LineChartDraw: React.FC<Props> = ({
  ...
  return (
    <AbsoluteFill name="Scene" style={{backgroundColor, padding: '78px 96px', fontFamily}}>
      <Interactive.Div
        name="Title"
        style={{fontSize: 50, fontWeight: 800, color: '#f2f4f8', letterSpacing: '-0.02em'}}
      >
        {title}
      </Interactive.Div>
      <Interactive.Div
      ...
      <svg width={W} height={H} style={{marginTop: 24, overflow: 'visible'}}>
        ...
        <circle cx={dotX} cy={dotY} r={9} fill="none" stroke="#0b0d14" strokeWidth={3} />
      </svg>
        ...
        {labels.map((l, i) => (
          <span
            ...
            style={{
              ...
              fontSize: 26,
              fontWeight: 600,
              color: i / (labels.length - 1) <= progress ? '#8e95a8' : '#3a3f4d',
            }}
          >
            {l}
          </span>
